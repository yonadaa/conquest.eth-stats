...
import { formatAllianceOnlyBlockie, formatOwner, formatTokens } from './Helpers';
...
const OwnersQueryWrapper = ({ orderBy }: { orderBy: string }) => {
  const { loading, error, data } = useQuery(OWNERS, { variables: { orderBy: orderBy } });
  ...
  return (
    <div>
      <p><i>{data.owners.length} results.</i></p>
